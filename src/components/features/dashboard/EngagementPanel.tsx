...
export default function EngagementPanel({ leadId }: { leadId: Id<"leads"> }) {
  ...
  return (
    <ul className="space-y-5">
      {engagements.map((e) => (
        <li key={e._id} className="border-b border-neutral-300 pb-4">
          <div className="flex items-baseline justify-between gap-4">
            <span className="text-body text-ink">{MODULE_LABEL[e.module] ?? e.module}</span>
            <span className="rounded-sm border border-neutral-300 px-2 py-0.5 text-caption text-slate">
              {e.status}
            </span>
          </div>
          ...
          {e.notes && <p className="mt-2 text-caption text-slate">{e.notes}</p>}

          {e.deliverables.length === 0 ? (
            <p className="mt-3 text-caption text-neutral-500">No work logged against it yet.</p>
          ) : (
                ...
                <li key={d._id} className="flex items-baseline justify-between gap-4">
                  <span className="text-caption text-ink">
                    ...
                    {d.producedAssessment && (
                      <span className="text-neutral-500"> · produced a coach assessment</span>
                    )}
                  </span>
